...
async def check_callback(callback):
    ...
    await bot.send_message(callback.message.chat.id, "News", reply_markup=markup)
...
async def check_callback(callback):
    ...
    markup = types.InlineKeyboardMarkup()
    button4 = types.InlineKeyboardButton(text="Назад", callback_data='back')
    button3 = types.InlineKeyboardButton(text="Включить оповещения о пробках в городе?", callback_data='NewsAboutCity')
